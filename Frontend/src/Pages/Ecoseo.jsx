...
const Ecoseo = () => {
  ...
  const displayDominateCards = Array.isArray(c?.dominate?.cards)
    ? c.dominate.cards.map((card) => ({ ...card, icon: resolveImage(card.icon) }))
    : [];

  const displayFaq = Array.isArray(c?.faqSection?.faq) ? c.faqSection.faq : [];
      ...
      {seoAgency && (
        <section className="w-full py-24 px-5 bg-white">
          <div className="max-w-[1200px] mx-auto">
            ...
            {seoAgency.heading && (
              <h2 style={{ fontSize: "34px" }}  className="text-center text-[38px] max-[769px]:text-[28px] font-heading font-extrabold text-[#1c1c1e] mb-8 uppercase leading-tight">
                {seoAgency.heading}
              </h2>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mt-12">
              ...
              <div className="space-y-6 text-[16px] leading-[1.8] text-[#4b5563] lg:pt-2">
                {seoAgency.rightParagraph && (
                  <p>{seoAgency.rightParagraph}</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="relative pl-8 border-l-2 border-[#e5e7eb] space-y-8">
                {impactData.items.map((item, index) => (
                  <div className="relative group" key={item.id || index}>
                    ...
                    <h3 className="text-lg font-bold text-slate-800 group-hover:text-[#e31e24] transition-colors duration-300 mb-1">
                      {item.title}
                    </h3>
