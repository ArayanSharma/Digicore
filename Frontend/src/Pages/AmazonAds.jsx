...
const AmazonAds = () => {
  ...
  const displayHelpCards = Array.isArray(helpCardsSection?.helpCards)
    ? helpCardsSection.helpCards.map((item) => ({ ...item, icon: resolveImage(item.iconUrl) }))
    : [];
        ...
        <section className="w-full py-[100px] px-5 bg-white border-b border-[#e5e7eb]">
          <div className="max-w-[1200px] mx-auto px-5">
            <div className="text-center mb-[60px]">
              ...
              {seoAgencySection.headingDescription && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-[50px]">
              <div>
                {seoAgencySection.problemTitle && <h3 className="text-[22px] font-heading font-bold text-[#2b2b2e] mb-[12px]">{seoAgencySection.problemTitle}</h3>}

                {seoAgencySection.problemDescription && (
                  <p className="text-[16px] leading-[1.8] text-[#4b5563] mb-[30px]">
                    {seoAgencySection.problemDescription}
                  </p>
                )}

                {seoAgencySection.solutionTitle && <h3 className="text-[22px] font-heading font-bold text-[#2b2b2e] mb-[12px]">{seoAgencySection.solutionTitle}</h3>}

                {seoAgencySection.solutionDescription && (
              ...
              <div>
                {seoAgencySection.additionalDescription && (
                  <p className="text-[16px] leading-[1.8] text-[#4b5563]">
                    {seoAgencySection.additionalDescription}
                  </p>
                )}
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {displayHelpCards.map((item, index) => (
                <div key={index} className="rounded-[32px] bg-white p-6 shadow-sm border border-slate-200 hover:shadow-md transition-shadow duration-300">
                  ...
                  {item.title && <h3 className="text-xl font-semibold text-slate-900 mb-3">{item.title}</h3>}
                  {(item.desc || item.description) && <p className="text-slate-600 leading-7">{item.desc || item.description}</p>}
                </div>
              ))}
            </div>
          </div>
