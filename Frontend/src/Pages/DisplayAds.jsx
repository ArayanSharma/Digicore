          ...
          <div className="max-w-[1200px] mx-auto flex justify-between items-center gap-[60px] lg:flex-row flex-col px-5">
            ...
            <div className="w-full lg:max-w-[50%] pt-10 lg:pt-0">
              <div className="relative border-l border-[#e5e7eb] pl-6 ml-3">
                {timelineSection.items.map((step, index) => (
                  <div key={step.id || index} className="relative mb-[40px] last:mb-0">
                    ...
                    <div>
                      {step.title && <h3 className="mb-[8px] text-[22px] max-[769px]:text-[19px] font-heading font-bold text-[#2b2b2e] leading-tight">{step.title}</h3>}
                      {(step.description || step.desc) && <p className="text-[15px] leading-[1.6] text-[#4b5563]">{step.description || step.desc}</p>}
                    </div>
                  </div>
                ))}
              </div>
