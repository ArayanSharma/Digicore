...
const GoogleAd = () => {
  ...
  const displayServices = Array.isArray(discoverServices?.services)
    ? discoverServices.services.map((s) => ({ ...s, icon: resolveImage(s.icon) }))
    : [];

  const defaultIcons = [Icon1, Icon2, Icon3, Icon4];
  const displayHelpCards = Array.isArray(dominate?.cards)
  ...
  const displayFaq = Array.isArray(faqSection?.items)
    ? faqSection.items
    : [];
          ...
          <div className="max-w-[1200px] mx-auto flex flex-col lg:flex-row justify-between items-center gap-[60px] lg:gap-[40px] px-5">
            ...
            {visibility.image && (
              <div className="flex-1 flex justify-center lg:justify-end">
                <img src={resolveImage(visibility.image)} alt="Graph" className="w-full max-w-[550px] h-auto object-cover rounded-2xl shadow-[0_15px_35px_rgba(0,0,0,0.06)] border border-[#e5e7eb]" />
              </div>
            )}
          </div>
        </section>
        ...
        <section className="w-full p-0 m-0 bg-white">
          <div className="grid grid-cols-1 lg:grid-cols-[40%_60%] gap-0 w-full">
            <div className="bg-gradient-to-br from-[#8b0000] to-[#1c1c1e] min-h-[350px] flex flex-col justify-center px-12 py-[80px] lg:pl-[120px]">
              {[performance.label1, performance.label2, performance.label3, performance.label4].map((word, i) => word && (
            ...
            {performance.image && (
              <div className="min-h-[390px] max-[992px]:min-h-[350px] max-[769px]:min-h-[250px] overflow-hidden">
                <img src={resolveImage(performance.image)} alt="SEO Performance" className="w-full h-full block object-cover" />
              </div>
            )}
          </div>
        </section>
          ...
          <div className="max-w-[1200px] mx-auto flex justify-between items-center gap-[60px] lg:flex-row flex-col px-5">
            {impactSection.image && (
              <div className="w-full max-w-[500px] lg:max-w-[45%]">
                <img src={resolveImage(impactSection.image)} alt="Impact" className="w-full block rounded-2xl shadow-[0_15px_35px_rgba(0,0,0,0.05)] border border-[#e5e7eb]" />
              </div>
            )}

            <div className="w-full lg:max-w-[50%] pt-10 lg:pt-0">
              <div className="relative border-l border-[#e5e7eb] pl-6 ml-3">
                {impactSection.timeline.map((item, index) => (
        ...
        <section className="bg-white py-16 sm:py-20">
          <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] items-start">
              {whyChooseDigicore.points?.length > 0 && (
                <div>
              ...
              {whyChooseDigicore.image && (
                <div className="rounded-[32px] bg-slate-50 p-5 shadow-sm">
                  <img
                    className="w-full rounded-[28px] object-cover"
                    src={resolveImage(whyChooseDigicore.image)}
                    alt="Graph"
                  />
                </div>
              )}
