...
const PAGE_SLUG = "instagram-ads";

export default function AdInsta() {
    ...
    if (data) {
      ...
      if (data.servicesSection) {
        setServicesSection({
          ...
          services: (data.servicesSection.services || []).map((item) => ({
            id: item.id || uid(),
            ...item,
          })),
        });
      ...
      if (data.whyChooseSection) {
        setWhyChooseSection({
          ...
          buttons: (data.whyChooseSection.buttons || []).map((item) => ({
            id: item.id || uid(),
            ...item,
          })),
        });
      ...
      if (data.timelineSection) {
        setTimelineSection({
          ...
          items: (data.timelineSection.items || []).map((item) => ({
            id: item.id || uid(),
            ...item,
          })),
        });
      ...
      if (data.faqSection) {
        setFaqSection({
          ...data.faqSection,
          faqItems: (data.faqSection.faqItems || []).map((item) => ({
            id: item.id || uid(),
            ...item,
          })),
        });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setStatus("saved");
      showToast("Instagram Ads page saved successfully");
  ...
  return (
    <div className="min-h-screen bg-bg-card py-8 px-4 md:px-8 lg:px-10 flex flex-col gap-6">
        ...
        <PageHeader title="Instagram Ads Page Content - Admin" description="Edit all Instagram Ads page sections from the admin panel." status={status} />
