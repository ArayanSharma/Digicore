...
const PAGE_SLUG = "Wix-Development";

export default function AdWixDev() {
  ...
  const removeCmsData = (id) => setCmsData(cmsData.filter((item) => item.id !== id));

  const updateCmsPoint = (cardId, index, value) =>
    setCmsData(
      cmsData.map((item) => {
  ...
  const addCmsPoint = (cardId) =>
    setCmsData(
      cmsData.map((item) =>
        item.id === cardId ? { ...item, points: [...(item.points || []), ""] } : item
      )
    );
  const removeCmsPoint = (cardId, index) =>
    setCmsData(
      cmsData.map((item) =>
        item.id === cardId
  ...
  const updateOurWorkBullet = (index, value) =>
    setOurWork({
      ...ourWork,
      bullets: ourWork.bullets.map((b, i) => (i === index ? value : b)),
    });
  ...
  const addFaq = () =>
    setFaqData([...faqData, { id: uid(), question: "", answer: "", isList: false, answerList: [] }]);
  const removeFaq = (id) => setFaqData(faqData.filter((item) => item.id !== id));

  const updateFaqPoint = (faqId, index, value) =>
    setFaqData(
      faqData.map((item) => {
  ...
  const addFaqPoint = (faqId) =>
    setFaqData(
      faqData.map((item) =>
        item.id === faqId ? { ...item, answerList: [...(item.answerList || []), ""] } : item
      )
    );
  const removeFaqPoint = (faqId, index) =>
    setFaqData(
      faqData.map((item) =>
        item.id === faqId
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setStatus("saved");
      showToast("Wix Development page saved successfully");
  ...
  return (
    <div className="min-h-screen bg-bg-card py-8 px-4 md:px-8 lg:px-10 flex flex-col gap-6">
        ...
        <PageHeader title="Wix Development Page Content - Admin" description="Edit all Wix Development page sections from the admin panel." status={status} />
      <form onSubmit={handleSave} className="w-full">
        ...
        <Section title="CMS Section" open={openSection === "cmsSection"} onToggle={() => toggle("cmsSection")}>
          ...
          <div className="md:col-span-2 space-y-3">
            <CardListHeader title="CMS Cards" onAdd={addCmsData} />
            {cmsData.map((item, index) => (
              <div key={item.id} className="p-4 bg-bg-secondary rounded-2xl border border-brand-primary/25 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <RemoveBtn onClick={() => removeCmsData(item.id)} />
                </div>
                <Field label="Icon">
                  <select
                    className={inputCls}
                    value={item.icon || CMS_ICON_OPTIONS[0]}
                    onChange={(e) => updateCmsData(item.id, "icon", e.target.value)}
                  >
                ...
                <Field label="Title">
                  <TextInput value={item.title} onChange={(e) => updateCmsData(item.id, "title", e.target.value)} />
                </Field>
                <Field label="Points">
            ...
            {faqData.map((item, index) => (
              <div key={item.id} className="p-4 bg-bg-secondary rounded-2xl border border-brand-primary/25 space-y-3">
                ...
                <label className="flex items-center gap-2 text-xs font-medium text-text-muted">
                  ...
                  />
                  Show answer as a list of bullet points
                </label>
