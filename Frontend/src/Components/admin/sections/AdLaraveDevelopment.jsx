...
const PAGE_SLUG = "laravel-development";

export default function AdLaraveDevelopment() {
  ...
  const addOurWorkBullet = () =>
    setOurWork((prev) => ({ ...prev, bullets: [...(prev.bullets || []), ""] }));
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setStatus("saved");
      showToast("Laravel Development page saved successfully");
  ...
  return (
    <div className="min-h-screen bg-bg-card py-8 px-4 md:px-8 lg:px-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-xl font-bold text-brand-primary">Laravel Development Page Content — Admin</h1>
            <p className="text-sm text-text-muted">Edit all Laravel Development page sections from the admin panel.</p>
          </div>
        </div>
      <form onSubmit={handleSave} className="w-full">
        ...
        <Section title="Testimonials Section" open={openSection === "testimonialHeader"} onToggle={() => toggle("testimonialHeader")}>
          ...
          <Field label="Video URL (YouTube or Direct File)">
            <TextInput value={testimonialHeader.videoUrl} onChange={(e) => setTestimonialHeader({ ...testimonialHeader, videoUrl: e.target.value })} />
          </Field>
