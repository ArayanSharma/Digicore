...
export default function AdAbout() {
  ...
  const load = useCallback(async () => {
    ...
    if (data) {
      ...
      if (data.aboutSection) setAboutSection((prev) => ({ ...prev, ...data.aboutSection }));
      if (data.betterSection) setBetterSection((prev) => ({ ...prev, ...data.betterSection }));
      if (data.stats) setStats(data.stats);
      if (data.contactBanner) setContactBanner((prev) => ({ ...prev, ...data.contactBanner }));
      if (data.industriesTitle) setIndustriesTitle(data.industriesTitle);
      if (data.industries) setIndustries(data.industries);
      if (data.missionSection) setMissionSection((prev) => ({ ...prev, ...data.missionSection }));
      if (data.visionSection) setVisionSection((prev) => ({ ...prev, ...data.visionSection }));
    }
    setLoading(false);
  }, []);
  ...
  const handleSave = async (e) => {
    ...
    if (process.env.NODE_ENV !== "production") console.debug("AdAbout: saving payload", payload);
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      <PageHeader title="About Page Content - Admin" description="Edit all About page sections from the admin panel." status={status} />
      <form onSubmit={handleSave} className="w-full max-w-[1306px] mx-auto">
        ...
        <div className="space-y-6 mt-6">
          <Section title="1. Hero / Banner" open={openSection === "hero"} onToggle={() => toggle("hero")}>
            ...
            <ImageInput label="Hero Image" value={hero.image} onChange={(e) => setHero({ ...hero, image: e.target.value })} />
