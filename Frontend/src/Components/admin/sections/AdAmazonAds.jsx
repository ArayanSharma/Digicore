...
const PAGE_SLUG = "amazon-ads";

export default function AdAmazonAds() {
  ...
  const [banner, setBanner] = useState({
    title: "Google Amazon Ads Management Agency",
    description:
      "At Digicore Inc., we help you Amazon Ads in front of your audience based on their browsing preferences. Experience the business growth through our strategically planned Google Amazon Ads management services.",
    ...
    services: [
      {
        id: uid(),
        iconUrl: "",
        title: "Google Amazon Ads Management",
        description:
          "With the help of Google Amazon Ads management, we strategically use targeted visuals, smart ad placements and effective data-driven optimization to help you reach the right audience. The objective of these services is to boost conversion, brand awareness and engagement across the Google display network.",
      },
      {
        id: uid(),
        iconUrl: "",
      ...
      },
      {
        ...
        description:
          "As a part of these campaigns, we capture high-quality prospects by working over Amazon Ads, optimized placements, compelling creatives. The objective of these campaigns is to drive form submissions, conversions, and inquiries while maximizing ROI.",
      },
      {
        id: uid(),
        iconUrl: "",
        ...
        description:
          "With the help of personalized Amazon Ads, we re-engage past website visitors by reminding them of your brand through these services. Besides, we encourage return visits, conversions and inquiries across the Google Displaay Network.",
      },
      {
        id: uid(),
        iconUrl: "",
        title: "Responsive Amazon Ads",
        description:
  ...
  const [faqSection, setFaqSection] = useState({
    ...
    faqItems: [
      {
        id: uid(),
        question: "In how much time can I expect Google Amazon Ads?",
        answer:
      ...
      {
        id: uid(),
        question: "Are Google Amazon Ads beneficial for remarketing?",
        answer:
          "Yes! Google Amazon Ads re-engage the visitors by showing them tailored ads of products & services they have browsed. This is one of the best ways to increase conversion opportunities and brand recall.",
      },
      {
        id: uid(),
        question: "Is it important to optimize Google Amazon Ads frequently?",
        answer:
  ...
  const updatePoint = (index, value) =>
    setWhyChooseDigicoreSection({
      ...
      points: whyChooseDigicoreSection.points.map((item, i) => (i === index ? value : item)),
    });
  const addPoint = () =>
    setWhyChooseDigicoreSection({
      ...whyChooseDigicoreSection,
  ...
  const load = useCallback(async () => {
    ...
    if (data) {
      ...
      if (data.helpCardsSection) setHelpCardsSection(data.helpCardsSection);
    }
    setLoading(false);
  }, []);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setStatus("saved");
      showToast("Amazon Ads page saved successfully");
  ...
  return (
    <div className="min-h-screen bg-bg-card py-8 px-4 md:px-8 lg:px-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-xl font-bold text-brand-primary">Amazon Ads Page Content — Admin</h1>
            <p className="text-sm text-text-muted">Edit all Amazon Ads page sections from the admin panel.</p>
          </div>
        </div>
      <form onSubmit={handleSave} className="w-full">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-[0_8px_24px_rgba(15,23,42,.06)] hover:shadow-[0_16px_36px_rgba(37,99,235,.10)] transition-all duration-300 p-8 space-y-8 mt-8">
          <Section title="1. Banner" open={openSection === "banner"} onToggle={() => toggle("banner")}>
          <Field label="Title">
        ...
        <Section title="6. Why Display Ads Section" open={openSection === "seoAgencySection"} onToggle={() => toggle("seoAgencySection")}>
          <Field label="Heading">
            <TextInput value={seoAgencySection.heading} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, heading: e.target.value })} />
          </Field>
          <Field label="Heading Description">
            <TextArea rows={5} value={seoAgencySection.headingDescription} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, headingDescription: e.target.value })} />
          </Field>
          <Field label="Problem Title">
            <TextInput value={seoAgencySection.problemTitle} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, problemTitle: e.target.value })} />
          </Field>
          <Field label="Problem Description">
            <TextArea rows={5} value={seoAgencySection.problemDescription} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, problemDescription: e.target.value })} />
          </Field>
          <Field label="Solution Title">
            <TextInput value={seoAgencySection.solutionTitle} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, solutionTitle: e.target.value })} />
          </Field>
          <Field label="Solution Description">
            <TextArea rows={5} value={seoAgencySection.solutionDescription} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, solutionDescription: e.target.value })} />
          </Field>
          <Field label="Additional Description">
            <TextArea rows={5} value={seoAgencySection.additionalDescription} onChange={(e) => setSeoAgencySection({ ...seoAgencySection, additionalDescription: e.target.value })} />
          </Field>
        </Section>
        ...
        <Section title="Help Cards Section" open={openSection === "helpCardsSection"} onToggle={() => toggle("helpCardsSection")}>
          <Field label="Section Heading">
            <TextInput value={helpCardsSection.heading} onChange={(e) => setHelpCardsSection({ ...helpCardsSection, heading: e.target.value })} />
          </Field>
          <Field label="Section Description">
            <TextArea rows={4} value={helpCardsSection.description || ""} onChange={(e) => setHelpCardsSection({ ...helpCardsSection, description: e.target.value })} />
          </Field>
          <LeadForm
            title="Help Cards"
            addLabel="Add Card"
            items={helpCardsSection.helpCards}
            onChange={(next) => setHelpCardsSection({ ...helpCardsSection, helpCards: next })}
            fields={[
          ...
          <div className="md:col-span-2 space-y-3">
            ...
            {faqSection.faqItems.map((item, index) => (
              <div key={item.id} className="p-4 bg-bg-secondary rounded-2xl border border-brand-primary/15 space-y-3">
                ...
                <Field label="Answer">
                  <TextArea rows={3} value={item.answer} onChange={(e) => updateFaq(item.id, "answer", e.target.value)} />
                </Field>
              </div>
            ))}
          </div>
