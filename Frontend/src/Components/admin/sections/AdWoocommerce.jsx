...
const PAGE_SLUG = "woocommerce-development";

export default function AdWoocommerce() {
  ...
  const removeCmsItem = (id) => setCmsData(cmsData.filter((item) => item.id !== id));
  ...
  const addFaqItem = () =>
    setFaqData([
      ...faqData,
      { id: uid(), question: "", isList: false, answer: "", answerList: [] },
    ]);
  const removeFaqItem = (id) => setFaqData(faqData.filter((item) => item.id !== id));
  const updateFaqPoint = (id, index, value) =>
    setFaqData(
      faqData.map((item) => {
        if (item.id !== id) return item;
  ...
  const addFaqPoint = (id) =>
    setFaqData(
      faqData.map((item) =>
        item.id === id ? { ...item, answerList: [...(item.answerList || []), ""] } : item
      )
    );
  const removeFaqPoint = (id, index) =>
    setFaqData(
      faqData.map((item) =>
        item.id === id
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setStatus("saved");
      showToast("WooCommerce page saved successfully");
  ...
  return (
    <div className="min-h-screen bg-bg-card py-8 px-4 md:px-8 lg:px-10 flex flex-col gap-6">
        ...
        <PageHeader title="WooCommerce Page Content - Admin" description="Edit all WooCommerce Development page sections from the admin panel." status={status} />
      <form onSubmit={handleSave} className="w-full">
        ...
        <Section title="2. Top Content" open={openSection === "topContent"} onToggle={() => toggle("topContent")}>
          <Field label="Title">
        ...
        <Section title="3. Help Cards" open={openSection === "helpCards"} onToggle={() => toggle("helpCards")}>
          <LeadForm
            title="Help Cards"
            addLabel="Add Card"
            items={helpCards}
        ...
        <Section title="4. Web Design Section" open={openSection === "webDesign"} onToggle={() => toggle("webDesign")}>
          <Field label="Title">
        ...
        <Section title="5. CMS Section" open={openSection === "cmsSection"} onToggle={() => toggle("cmsSection")}>
          <Field label="Title">
            ...
            {cmsData.map((item, index) => (
              <div key={item.id} className="p-4 bg-bg-secondary rounded-2xl border border-brand-primary/25 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <RemoveBtn onClick={() => removeCmsItem(item.id)} />
                </div>
                <Field label="Icon">
                  <select
                    className={inputCls}
                    value={item.icon || CMS_ICON_OPTIONS[0]}
                    onChange={(e) => updateCmsItem(item.id, "icon", e.target.value)}
                  >
                ...
                <Field label="Title">
                  <TextInput value={item.title} onChange={(e) => updateCmsItem(item.id, "title", e.target.value)} />
                </Field>
                <Field label="Points">
        ...
        <Section title="6. Our Work Section" open={openSection === "ourWork"} onToggle={() => toggle("ourWork")}>
          <Field label="Title">
        ...
        <Section title="7. Built Section" open={openSection === "builtSection"} onToggle={() => toggle("builtSection")}>
          <Field label="Title">
        ...
        <Section title="9. Process Section" open={openSection === "processHeader"} onToggle={() => toggle("processHeader")}>
          <Field label="Title">
        ...
        <Section title="10. Testimonials Section" open={openSection === "testimonialHeader"} onToggle={() => toggle("testimonialHeader")}>
          <Field label="Title">
          ...
          <Field label="Video URL (YouTube link)">
            <TextInput placeholder="YouTube video URL" value={testimonialHeader.videoUrl} onChange={(e) => setTestimonialHeader({ ...testimonialHeader, videoUrl: e.target.value })} />
          </Field>
        ...
        <Section title="11. Contact Banner" open={openSection === "contactBanner"} onToggle={() => toggle("contactBanner")}>
        ...
        <Section title="12. FAQ Section" open={openSection === "faqHeader"} onToggle={() => toggle("faqHeader")}>
          <Field label="Title">
              ...
              <div key={item.id} className="p-4 bg-bg-secondary rounded-2xl border border-brand-primary/25 space-y-3">
                ...
                <label className="flex items-center gap-2 text-sm text-text-secondary">
                  <input
                    type="checkbox"
                    checked={!!item.isList}
                    onChange={(e) => updateFaqItem(item.id, "isList", e.target.checked)}
                  />
                  ...
                  <Field label="Answer Points">
                    <div className="space-y-2">
                      {(item.answerList || []).map((point, pointIndex) => (
                        <div key={pointIndex} className="flex items-center gap-3">
                          <TextInput value={point} onChange={(e) => updateFaqPoint(item.id, pointIndex, e.target.value)} />
                          <RemoveBtn onClick={() => removeFaqPoint(item.id, pointIndex)} />
                        </div>
                      ))}
                      <AddRowBtn label="Add Point" onClick={() => addFaqPoint(item.id)} />
                    </div>
                  </Field>
                ) : (
