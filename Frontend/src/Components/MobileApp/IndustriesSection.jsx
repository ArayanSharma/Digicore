...
export default function IndustriesSection({ header, industries }) {
  const eyebrow = header?.eyebrow || "Industries We Serve";
  const title = header?.title || "Domain Expertise Across Every Sector";
  const description = header?.description || "We've shipped mobile products for founders and enterprises across a wide range of industries.";

  const items = industries?.length ? industries : DEFAULT_INDUSTRIES;
